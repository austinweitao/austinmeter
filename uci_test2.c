...
#define	    METER_LASTVALUE_CONFIG_FILE "/etc/config/meter_lastvalue"
...

{
    ...
    ctx = uci_alloc_context(); 
    if (UCI_OK != uci_load(ctx, METER_LASTVALUE_CONFIG_FILE, &pkg)){
	ret = -1;
	printf("meter_config error.\n");
        goto cleanup; 
    }
    ...
    (void) fprintf(stderr,"\n---------------initializing meter attribute last value----------\n");
    int n=0;
    for (l=head; l; l=l->next)
    {
        addr=(Meter*) l->data;
	uci_foreach_element(&pkg->sections, e){
	    ...
	    if(!strcmp("meter_lastvalue",s->type)) //this section is a meter_lastvalue
	    {
